...
import { patch } from '@/common/utils/request/patch';
...
describe('Тесты для request utils. patch()', () => {
  ...
  it('Возвращает результат выполнения утилиты sendRequest()', () => {
    const callResult = patch(url, {}, reqSettings);
  ...
  it('Вызывает утилиту sendRequest() c правильными параметрами', () => {
    ...
    patch(url, data, requestSettings);

    expect(mockFunction).toBeCalledTimes(1);
    expect(mockFunction.mock.calls[0][0]).toBe(url);
    expect(mockFunction.mock.calls[0][1]).toBe(HTTP_METHODS.PATCH);
