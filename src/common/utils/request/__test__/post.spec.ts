...
import { post } from '@/common/utils/request/post';
...
describe('Тесты для request utils. post()', () => {
  ...
  it('Возвращает результат выполнения утилиты sendRequest()', () => {
    const callResult = post(url, {}, reqSettings);
  ...
  it('Вызывает утилиту sendRequest() c правильными параметрами', () => {
    ...
    post(url, data, requestSettings);

    expect(mockFunction).toBeCalledTimes(1);
    expect(mockFunction.mock.calls[0][0]).toBe(url);
    expect(mockFunction.mock.calls[0][1]).toBe(HTTP_METHODS.POST);
