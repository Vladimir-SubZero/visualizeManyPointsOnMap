...
import { del } from '@/common/utils/request/delete';
...
describe('Тесты для request utils. put()', () => {
  ...
  it('Возвращает результат выполнения утилиты sendRequest()', () => {
    const callResult = del(url, {}, reqSettings);
  ...
  it('Вызывает утилиту sendRequest() c правильными параметрами', () => {
    ...
    del(url, data, requestSettings);

    expect(mockFunction).toBeCalledTimes(1);
    expect(mockFunction.mock.calls[0][0]).toBe(url);
    expect(mockFunction.mock.calls[0][1]).toBe(HTTP_METHODS.DELETE);
